...
/**
 * This is used for adjusting robot angle from GRIP values 
 */
...
//	Constants for known variables
    ...
    protected void initialize() {
		...
		NetworkTable.setIPAddress("roborio-58-frc.local");
...
//		make sure the java process quits when the loop finishes
    ...
    protected void execute() {
    	...
    	//get network values
    	centerXArray = grip.getNumberArray("centerX", error);
...
//		only run for the specified time
	...
	public static double returnCenterX(){
		double[] defaultValue = new double[0];
			...
			if(!Robot.filterContoursOutput.isEmpty() && Robot.filterContoursOutput.size() >= 2){
				Rect r = Imgproc.boundingRect(Robot.filterContoursOutput.get(1));
				Rect r1 = Imgproc.boundingRect(Robot.filterContoursOutput.get(0)); 
